...
class AsciiRenderer:
    ...
    def render(self, stdscr: curses.window, path: Optional[List[Tuple[int, int]]] = None) -> None:
        ...
        for y in range(h):
            for x in range(w):
                ...
                try:
                    stdscr.addstr(py, px, "+")
                    if val == 15: stdscr.addstr(py + 1, px + 1, "███", curses.color_pair(2))
                    if val & 1: stdscr.addstr(py, px + 1, "---")
                    if val & 8: stdscr.addstr(py + 1, px, "|")
        ...
        for x in range(w):
            try: stdscr.addstr(h * 2 + offset_y, x * 4, "+---")
            except curses.error: pass
        try: stdscr.addstr(h * 2 + offset_y, w * 4, "+")
        except curses.error: pass
    ...
    def handle_move(self, direction, stdscr):
        ...
        if direction not in move_map: return
        ...
        while True:
            ...
            if self.play_mode:
                ...
                if key in (ord('p'), ord('P')):
                    ...
                    if show_solution: self.animate_path(stdscr, path)
                elif key in (ord('r'), ord('R')): self.regenerate_game()